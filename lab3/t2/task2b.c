...
int main(){
    ...
    while(1){
        ...
        c = fgetc(stdin) - 49;
        fgetc(stdin);
        if(c<=sizeof(menu)/sizeof(menu[0]) && c>=0){
            ...
            if(c == 4){
                ...
                char buffer[10000] = "";
                //char input[50];
                scanf("%s",suspected_filename);
                fgetc(stdin);
                FILE *in;
                in= fopen(suspected_filename,"rb");
                if (in != NULL) {
                    fseek(in, 0, SEEK_END);
                    int size = ftell(in);
                    fseek(in, 0, SEEK_SET);
...
//                fgetc(stdin);
                int sbyte,size2;
                char start_byte_buff[5]="";
                char size_sig_buff[5]="";
                printf("Please enter the starting byte location:\n");
                fgets(start_byte_buff,5,stdin);
                //scanf("%d",&sbyte);
                sbyte = strtol(start_byte_buff,(char **)NULL,10);
                printf("Please enter the size of the virus signature:\n");
                fgets(size_sig_buff,5,stdin);
                size2 = strtol(size_sig_buff,(char **)NULL,10);
                //scanf("%d",&size2);
                //FILE *filename = fopen("infected","ab");
                //char *filename = "infected"; //nowhere is it said that we are to ask the user
                kill_virus(suspected_filename,sbyte,size2);
                printf("DONE.\n");
...
void kill_virus(char *fileName, int signatureOffset, int signatureSize){
    ...
    {
        //unsigned char *NOP = "\0";
        unsigned char toW[signatureSize];
        for(int i = 0; i < signatureSize; i++){
            toW[i] = 144;
        }
        fseek(in,signatureOffset,SEEK_SET);
        fwrite(toW,sizeof(unsigned char),signatureSize,in);
        fclose(in);
    }
