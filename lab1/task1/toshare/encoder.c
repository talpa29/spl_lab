...
int main(int argc,char** argv){
	...
	if(sizeof(argv) <= 1){
		...
		for(i=1;i<argc;i++){
			...
			if(key[1] =='o' && key[0] =='-'){
				oflag = 1;
				//char file1[strlen(argv[i] - 1)];
				for(int j = 2; j < strlen(argv[i]);j++){
					file1[j-2] = argv[i][j];
				}
				file1[strlen(argv[i])] = '.';
				file1[strlen(argv[i])+1] = 't';
				file1[strlen(argv[i])+2] = 'x';
				file1[strlen(argv[i])+3] = 't';
				writeTO = fopen(file1,"w"); // w for read mode
				if(file1 == NULL){
